...
| **Listing assets** | ⚠️ Partly | All five 1280×800 screenshots are committed under `store/screenshots/` and current, generated by `store/capture.mjs` + `store/compose.mjs` from the real extension (see `store/README.md`). Still missing: the 440×280 small promo tile, the detailed description, category and support contact. |
...
4. Produce the 440×280 small promo tile, the detailed description, category and
   support contact. All five screenshots are ready in `store/screenshots/`;
   specs and upload steps are in `store/README.md`. *(Chrome listing)*
