...
| **Manifest field limits** | ✅ | `name` 11 chars (limit 75); `version` `0.1.0` is a valid 1–4 part dotted integer. `description` is now the placeholder `__MSG_extDescription__`, which the browser substitutes — the limit applies to the *resolved* string, and both catalogs clear it: en 100 chars, uk 132 chars (limit 132, uk is at the limit — re-measure before any edit). |
