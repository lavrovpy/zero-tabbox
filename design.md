...
### D14. English and Ukrainian: one set of catalogs, two delivery paths, two regimes
Chosen: standard WebExtension catalogs at the repo root (`_locales/{en,uk}/messages.json`, `en` the fallback for every lookup and the manifest's `default_locale`), used twice over. The UI `import`s them and Bun inlines the JSON into each bundle at build time; `build.mjs` *also* copies the same directory verbatim into `dist/<browser>/_locales`. This looks like duplication and a later reader will try to delete one half — neither half is removable:

- The **copy in `dist`** is the only thing the browser can read. It resolves `manifest.base.json`'s `__MSG_extDescription__` / `__MSG_commandEndDayNow__`, and it is what `addons-linter` and AMO check against `default_locale`. Remove it and the extension's own description becomes the literal string `__MSG_extDescription__`.
- The **inlined import** is what the UI reads. Fetching the catalog at runtime (`runtime.getURL` + `fetch`) would resolve *after* first paint — precisely the flash `data-i18n-pending` exists to prevent — and would add a failure mode in which a page has no strings at all. An import has neither: the strings are in the bundle before the page exists.

**Two regimes, and their visible consequence.** The manifest follows the BROWSER's UI language; the UI follows the `locale` SETTING (`'auto' | 'en' | 'uk'`, default `'auto'` → `uk` when the browser UI language starts with `uk`, else `en`; a prefix test because the real tag is `uk` on some builds and `uk-UA` on others, and exact matching would leave half of Ukrainian users on English). The two cannot be unified in either direction. `chrome.i18n.getMessage()` reads the browser UI language and admits no per-call override, so it cannot serve a user who wants Ukrainian strings in an English browser — which is why the UI has its own resolver (`src/i18n.ts`) rather than the platform's. And the manifest is substituted by the browser before any of our code runs, so no setting of ours can reach it. The consequence is therefore expected and permanent: **an English `chrome://extensions` listing beside a Ukrainian popup.** That is platform-inherent, not a bug to file, and not to be "fixed" by routing UI strings through `chrome.i18n` — doing so silently deletes the setting.

**Plurals: `Intl.PluralRules` over flat sibling keys.** A plural group is a set of underscore-suffixed siblings (`noticeMessage_one`, `_few`, `_many`, `_other`), not a nested object. True nesting is rejected outright: `messages.json` requires a `message` property on every top-level entry, so `{"options": {"noticeOptionAriaLabel_few": …}}` fails validation (`addons-linter` reports `NO_MESSAGE`). Dotted *flat* names are a subtler case and worth stating precisely, because the obvious summary is wrong: `addons-linter` accepts them, and the UI never routes through `chrome.i18n`, so `options.noticeOptionAriaLabel_few` would in fact resolve for every UI key. It is only the two names the *browser* substitutes (`__MSG_extDescription__`, `__MSG_commandEndDayNow__`) that are bound by Chrome's documented `A-Za-z0-9_@` charset, where a dot risks the reference rendering literally. Flat camelCase is chosen so one rule covers every key rather than two rules split by who resolves them. `t()` asks `Intl.PluralRules` for the form and looks up `` `${key}_${form}` ``, falling back through `_other`, `_many`, the bare key, and then the English catalog. Over integers `uk` yields exactly `one`/`few`/`many` (0 → many, 21 → one, 22 → few) and `en` exactly `one`/`other`, so the selected key normally exists; the extra candidates cover a non-integer count (for which `uk` answers `other`, a form the Ukrainian catalog has no reason to carry) and the fallback into an English catalog that has no `few`/`many` at all. This is the whole of the 8-key delta between the catalogs: 78 keys in `en`, 86 in `uk`, being eight plural groups that carry a third form.

**No `description` fields, in either catalog.** The format allows one per entry and its purpose is translator context, but the catalogs are inlined into all four bundles: annotating them cost ~61 KB of prose per bundle — more than half of the extension's JavaScript — which the service worker re-parsed on every alarm wake to render a two-digit badge. Stripping them took `dist` JS from 448 KB to 192 KB and `background.js` from 120 KB to 55 KB. The context they carried belongs in the key names, which are surface-prefixed and explicit for that reason (`optionsNoticeOptionAriaLabel_few`, `onboardingTerm3`); genuinely load-bearing translation decisions — the «зараз» in `popupEndDayNow`, the always-«о» rule — are recorded here instead, where they cost nothing at runtime.

**A counted sentence is one message with the number inline, never a fragment glued to a formatted count.** Ukrainian inflects the verb *before* the numeral as well as the noun after it — «Залишилася 1 хвилина» / «Залишилися 2 хвилини» / «Залишилося 5 хвилин» — so the sentence cannot be split into "number" and "translatable tail": the halves are not independently translatable, and the English-shaped split bakes in an agreement rule Ukrainian does not have. Every counted string is therefore a whole sentence carrying `$COUNT$`, and `t()` makes a numeric `count` both the plural selector and the placeholder value so a caller never passes the number twice.

**Accepted imperfection: the Ukrainian copy always writes «о» before a time.** Correct Ukrainian takes «об» before a vowel — 11:00 is «об 11:00» — and «о» everywhere else. Choosing between them means inspecting the rendered hour from inside a string the catalog owns, i.e. a second plural-rules-shaped mechanism (a preposition selector, its own sibling keys, its own fallbacks) for one hour of the day. Not worth it: the catalogs always say «о», which is right for 23 of the 24 hour families and mildly wrong for 11:00. Recorded here so it reads as a decision rather than an oversight.

**`ROOT_FOLDER` in `bookmarks.ts` stays the literal `'zero-tabbox'`.** The brand is never localized anywhere, but the bookmark folder is the case with teeth: D12 is explicit that the extension keeps *no reference* to the bookmarks it writes, so if a Ukrainian translation of the folder name shipped, the extension would have no way to find — let alone migrate — the folders every existing user already has. Yesterday's saved tabs would silently stop being appended to and start accumulating under a second root. The escape hatch outranks the translation.

**Anti-flicker, failing open.** Every extension page ships with the English text visible in the markup and annotated (`data-i18n`, `data-i18n-attr`, `data-i18n-title`), and carries `data-i18n-pending` on `<html>`, which `theme.css` uses to hide `body`. `localize()` reads the setting, walks the DOM and clears the attribute — in a `finally`, so a throw anywhere in that path reveals the untranslated English page instead of leaving a permanently blank one. English in the markup is what makes failing open worth doing: the fallback is a readable page in the fallback language, not empty boxes.

**Changing the language writes `settings`, which wakes the background.** The `storage.onChanged` listener in `background.ts` fires on any `settings` write and calls `reconcile('settings-changed')`, so picking a language re-arms every alarm. That is harmless — the settings-changed path fast-forwards `lastAutoCutoffId` and never sweeps retroactively (D3) — but it is stated here because the tempting "optimisation" is to narrow the listener to the scheduling fields, which would break the real schedule triggers for the sake of one redundant re-arm.

Alternatives rejected: `chrome.i18n.getMessage()` for the UI as well (no per-call override — the language setting could not exist, and "the browser is in English" is exactly the situation a Ukrainian speaker installs this in); runtime-fetched catalogs (the flash and the failure mode above); an ICU MessageFormat library (a dependency and a parser to select among eight plural groups that `Intl.PluralRules`, already in the platform, selects for free); shipping Ukrainian only in the manifest and leaving the UI English (the manifest is the one surface the user reads once and never again).
