...
**Plurals: `Intl.PluralRules` over flat sibling keys.** A plural group is a set of underscore-suffixed siblings (`noticeMessage_one`, `_few`, `_many`, `_other`), not a nested object, because `messages.json` restricts message names to `A-Za-z0-9_@`: a dot is rejected by the browser and by addons-linter, so nesting was never on the table. `t()` asks `Intl.PluralRules` for the form and looks up `` `${key}_${form}` ``, falling back through `_other`, `_many`, the bare key, and then the English catalog. Over integers `uk` yields exactly `one`/`few`/`many` (0 → many, 21 → one, 22 → few) and `en` exactly `one`/`other`, so the selected key normally exists; the extra candidates cover a non-integer count (for which `uk` answers `other`, a form the Ukrainian catalog has no reason to carry) and the fallback into an English catalog that has no `few`/`many` at all. This is the whole of the 8-key delta between the catalogs: 78 keys in `en`, 86 in `uk`, being eight plural groups that carry a third form.
