...
### D7. Onboarding = one dedicated screen, once
`onInstalled` with `reason === 'install'` opens `ui/onboarding.html` — a single screen that states the contract as five numbered terms (every tab closes; nothing is stored; no undo/archive/snooze; closing the browser doesn't dodge it; no data leaves the browser) with the real first cutoff in the headline. Both buttons accept the contract: "I understand. Start at 18:00." closes the page, "Pick a different time" navigates the same tab to the options page. There is deliberately no decline button — declining is the browser's own uninstall flow. The page writes `onboarded` on view and is never opened again automatically; updates never open anything. The options page keeps the standing one-sentence contract in its header but no longer plays the onboarding role (this supersedes the earlier options-page-with-highlight approach).
...
The accent (ember) role is reserved for the commitment controls — "End day now" and the onboarding "I understand" button: the actions that accept the consequence should be the visually loudest, and nothing else may borrow that weight (selected states use the soft accent tint, not the solid fill). Color is emphasis, never the message: both labels state the action in words. This replaces the earlier red `--destructive` role; the product's one "destructive" act is its whole point, and the brand accent owning it is the redesign's statement.
