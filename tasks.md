...
- [ ] 6.3 First-install onboarding page (`ui/onboarding.html`, design.md D7): five terms, headline naming the first cutoff, "I understand" (closes the tab) and "Pick a different time" (same-tab to options); writes `onboarded` on view; `onInstalled` (`reason === 'install'`) opens it once, updates open nothing
